...
- **字幕播放模式**：底部「字幕」按钮一键进入全屏字幕播放 —— 全黑背景、歌词大字居中显示，底部小拉条调节字幕字号（24–100sp）、左侧竖向拉条调节字幕亮度（20–100%，夜间防刺眼），均自动记忆；右上角 ✕ 下方为播放/暂停按钮（同步控制网页音乐，暂停时字幕隐藏）；控制条 5 秒自动隐藏、点击屏幕唤出；✕ 按钮或返回键退出
...
| 字幕 | 进入/退出字幕播放模式（全屏黑底、字幕居中、底部拉条调字号、左侧拉条调亮度、播放/暂停同步网页音乐、防息屏） |
...
- 应用包名：`com.folatkikoeru.app`（v1.10 / versionCode 10）
